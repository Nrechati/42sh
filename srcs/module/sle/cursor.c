...
static inline void cursor_move(t_interface *itf)
{
	...
	if (itf->window.point_cursor
					> vct_len(itf->line))
		return ;
	index_to_coord(itf, get_prompt_length(&itf->prompt)
						+ itf->window.point_cursor, &coord);
...
void     move_cursor(t_registry *shell)
{
    ...
    if (rd_flag & RD_CEND)
    {
		index_to_coord(&shell->interface,
						get_prompt_length(&shell->interface.prompt)
						+ vct_len(shell->interface.line), &coord);
    ...
    {
		index_to_coord(&shell->interface,
						get_prompt_length(&shell->interface.prompt), &coord);
		move_cursor_to_coord(&shell->interface, coord.x, coord.y);
