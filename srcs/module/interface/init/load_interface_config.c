...
static int8_t		fetch_terminal_info(t_registry *shell)
{
	...
	if ((term_name = getenv("TERM")) == NULL || ft_strequ(term_name, ""))
	{
		...
		add_intern_var(shell, INT_TERM, INT_TERM_DFLT_VALUE);
	}
	else
		add_intern_var(shell, INT_TERM, term_name);
...
int8_t				fill_interface_related_internals(t_registry *shell)
{
	...
	if (add_intern_var(shell, INT_PS2, INT_PS2_VALUE) == FAILURE)
		return (FAILURE);
	if (add_intern_var(shell, INT_PS3, INT_PS3_VALUE) == FAILURE)
		return (FAILURE);
	if (add_intern_var(shell, INT_PS4, INT_PS4_VALUE) == FAILURE)
		return (FAILURE);
	return (SUCCESS);
}
